...
/*--------------------------------------------------
 ...
 |
 | E-mail: daffra.claudio@gmail.com
 | URL:  
 | Last Changed: 2017/08/2017
 | Added Code mirror Support
 | 
 +--------------------------------------------------
...
// +--------------------------------------------------
// | Interface 1234 
// | modificato da : claudio daffra 
// | daffra.claudio@gmail.com 15/3/2017
// | incluseo codemirror addon
// +--------------------------------------------------
...
function dir_list_form() {
    ...
    <script language=\"Javascript\" type=\"text/javascript\">
        ...
        function openModalWindow(url,title,w,h,reloadOnClose ) /* 1234 - MODIFICA/GUARDA : FILE */
		{
			//alert ( 'OPEN MODAL WINDOW : claudio daffra');
            ...
            document.getElementById(\"modalIframe\").style.width = (w-20)+'px';
            document.getElementById(\"modalIframe\").style.height = (h-20)+'px';
    ...
    if ($opdir = @opendir(fs_encode($current_dir))) {
		...
		{
			// 12345 Open file readOnly
            if(confirm('".uppercase(et('View'))." \\' '+arg+' \\' ?')) 
				openModalWindow(
				'".addslashes($fm_path_info["basename"])."?action=4&current_dir=".addslashes($current_dir)."&filename='+escape(arg),'".et("View")." '+escape(arg),
				800,
				600);
        }
        function rename(arg){
...
function edit_file_form()
{
    ...
    fclose($fh);
    html_header(); // 1234 - header : include 
    echo "<body id='fmwbody' marginwidth=\"0\" marginheight=\"0\">
    
	<table  border=0 cellspacing=0 cellpadding=5  style=\"width:95%;height:95%;padding:5px;table-layout: fixed\">
		<form name=\"edit_form\" action=\"".$fm_path_info["basename"]."\" method=\"post\">
			...
			<tr><td colspan=2><textarea id='xyz' name=file_data style='position:relative;width:95%;height:95%;border: 1px solid #ccc;'>".html_encode($file_data)."</textarea></td></tr>
	...
	<script>
				// 12345 codeMirror set
				window.editor = CodeMirror.fromTextArea( document.getElementById('xyz') , 
				{
					enter : '' ,
					lineNumbers: true,
					matchBrackets: true,
				...
				var ext = '".$file."'
				//alert(ext);
				window.editor.setOption('mode', 'text/plain');
				if ( ext.search('.js') != -1 ) window.editor.setOption('mode', 'text/javascript');
				if ( ext.search('.css') != -1 ) window.editor.setOption('mode', 'text/css');
				if ( ext.search('.php') != -1 ) window.editor.setOption('mode', 'application/x-httpd-php');				
				if ( ext.search('.html') != -1 ) window.editor.setOption('mode', 'htmlmixed');		
				
				
				window.editor.setCursor ( { line : 0 , ch:0 } ) ;
				var wwww=document.getElementById('fmwbody').width ;
				var hhhh=document.getElementById('fmwbody').height ;

				window.editor.setSize ( wwww*0.95,640 ) ;				
